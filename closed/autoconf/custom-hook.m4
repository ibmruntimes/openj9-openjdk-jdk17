...
[
  ...
  OPENJDK_TAG=
  for tag in `git -C $TOPDIR tag --list "jdk-13*+*" | sed -e "/_openj9/d" -e "s:^:refs/tags/:"` ; do
  ...
  if test x$OPENJDK_TAG != x ; then
    ...
    OPENJDK_TAG=`git -C $TOPDIR tag --points-at $OPENJDK_TAG | grep "jdk-13.*+" | grep -v _openj9 | $tag_sort1 | $tag_sort2 | tail -1`
  fi
