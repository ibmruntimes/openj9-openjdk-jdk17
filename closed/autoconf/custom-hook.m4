...
[
  ...
  OPENJDK_TAG=`git -C $TOPDIR tag --points-at HEAD | $GREP "jdk-13.*+" | $GREP -v _openj9 | $tag_sort1 | $tag_sort2 | $TAIL -1`
  ...
  if test x$OPENJDK_TAG = x ; then
    ...
    tagged_sha=`($ECHO filler ; git -C $TOPDIR rev-list '--tags=jdk-13*+*' '--exclude=*_openj9*' --topo-order --no-walk HEAD) | $SED -e "1,/$head_sha/d" | $HEAD -1`

    if test x$tagged_sha != x ; then
      # Select the latest tag, like above.
      OPENJDK_TAG=`git -C $TOPDIR tag --points-at $tagged_sha | $GREP "jdk-13.*+" | $GREP -v _openj9 | $tag_sort1 | $tag_sort2 | $TAIL -1`
    fi
  fi
