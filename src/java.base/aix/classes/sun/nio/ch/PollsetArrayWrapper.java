 ...
 *
 * Manipulates a native array of pollfd structures on AIX:
 *
 ...
 *
 * When a user calls select method, buffered events are flushed to the OS in order for the pollset to process.
...
class PollsetArrayWrapper {
    ...
    static final int CTL_EVENTS_ARRAY_SIZE = NUM_POLLCTLEVENTS * SIZE_POLLCTL;
    ...
    private final Object pollsetUpdaterLock = new Object();

    private int pollsetUpdaterCount;

    // Keeps track of events count, to be polled.
    private int pollsetNumEvents;
    ...
    private final int pollsetFD;

    // File descriptor to write for interrupt.
    private int interruptFD;

    // Number of updated pollfd entries.
    private int updated;

    // Returns number of updated pollfd entries.
    int getUpdated() {
        return updated;
    }
    ...
    PollsetArrayWrapper() {
         ...
         * to the pollset is bounded by OPEN_MAX as defined in <sys/limits.h>.
         */
        ...
        pollCtlArrayCurrent = new AllocatedNativeNode(CTL_EVENTS_ARRAY_SIZE, true);
    ...
    /*
     * On a 64-bit machine it returns -1 (when it exceeds int limit).
     ...
     * methods. These should be synchronized against pollsetUpdaterLock.
     * Currently this is not synchronized as caller of these methods is only
     * updateRegistration method and callers of these methods are taking care
     * of synchronization.
     */
    ...
    private void putCtlFD(int i, int fd) {
        int offset = SIZE_POLLCTL * i + CTL_FD_OFFSET;
        pollCtlArrayCurrent.putInt(offset, fd);
    }

    /**
     * Clear the poll_ctl entry at index i before setting its fields.
     */
    private void clearCtlEntry(int i) {
        int offset = SIZE_POLLCTL * i;
        // zero the entire poll_ctl structure
        for (int j = 0; j < SIZE_POLLCTL; j++) {
            pollCtlArrayCurrent.putByte(offset + j, (byte) 0);
        }
    }
    ...
    void setInterest(int fd, int mask) {
        synchronized (pollsetUpdaterLock) {
    ...
    private void flushBulkPollCtlEvents() {
        synchronized (pollsetUpdaterLock) {
            ...
            while (remainingEvents > 0) {
                eventsToBeFlushed = Math.min(remainingEvents, NUM_POLLCTLEVENTS);
    ...
    /* Be careful while calling this function. This should be synchronized
     * against pollsetUpdaterLock. Currently this is not synchronized as the
     * callers of this method are synchronized against pollsetUpdaterLock.
     */
    private void updateRegistration(short cmd, short events, int fd) {
        if (pollsetUpdaterCount == NUM_POLLCTLEVENTS) {
        ...
        clearCtlEntry(pollsetUpdaterCount);
        putCtlCmdOps(pollsetUpdaterCount, cmd);
        putCtlEventsOps(pollsetUpdaterCount, getAIXEvent(events));
        putCtlFD(pollsetUpdaterCount, fd);
        pollsetUpdaterCount += 1;
        pollsetNumEvents += 1;
    }
    ...
    private static short getPollRevents(short revents) {
        short aixRevents = 0;
        if ((revents & POLLIN) != 0) {
            aixRevents |= Net.POLLIN;
        }
        if ((revents & POLLOUT) != 0) {
            aixRevents |= Net.POLLOUT;
        }
        if ((revents & POLLERR) != 0) {
            aixRevents |= Net.POLLERR;
        }
        if ((revents & POLLHUP) != 0) {
            aixRevents |= Net.POLLHUP;
        }
        if ((revents & POLLNVAL) != 0) {
            aixRevents |= Net.POLLNVAL;
        }
        return aixRevents;
    }
